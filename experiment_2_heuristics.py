...
SUPPORTED_EXTS = {".graphml", ".gml", ".gexf", ".gpickle", ".pkl", ".pickle", ".edgelist", ".edges", ".txt"}

@dataclass
class ExperimentConfig:
    dataset_paths: List[str] = field(default_factory=lambda: ["real-world-dataset", "synthetic-dataset"])
    simulate_iters: int = 10
    methods: List[str] = field(default_factory=lambda: ["TB"])
...
def try_load_graph(path: str) -> Optional[nx.Graph]:
    ...
    try:
        ...
        elif ext in {".edgelist", ".edges", ".txt"}:
...
def generate_instances(config: ExperimentConfig) -> List[Tuple[str, int, nx.Graph]]:
    instances = []
    paths = config.dataset_paths or []
    logger.info(f"Loading datasets from: {paths}")
    t0 = time.perf_counter()
    for root_path in paths:
        if not os.path.isdir(root_path):
            logger.warning(f"Dataset path not found: {root_path}")
            continue
        for dirpath, _, filenames in os.walk(root_path):
            for fname in filenames:
                if os.path.splitext(fname)[1].lower() in SUPPORTED_EXTS:
                    fpath = os.path.join(dirpath, fname)
                    G = try_load_graph(fpath)
                    if G is None:
                        continue
                    n = G.number_of_nodes()
                    tag = os.path.basename(root_path)
                    instances.append((tag, n, G))
    logger.info(f"Loaded {len(instances)} instances in {time.perf_counter() - t0:.3f}s")
    return instances
...
if __name__ == "__main__":
    ...
    parser.add_argument("--simulate-iters", type=int)
    parser.add_argument("--methods", nargs="+", choices=["SA", "TB"])
    parser.add_argument("--sa-num-sweeps", type=int)
    parser.add_argument("--sa-num-reads", type=int)
    parser.add_argument("--sa-seed", type=int)
    parser.add_argument("--tb-tenure", type=int)
    parser.add_argument("--tb-num-reads", type=int)
    parser.add_argument("--tb-seed", type=int)
    ...
    cfg = ExperimentConfig(
        dataset_paths=args.datasets if args.datasets is not None else default_cfg.dataset_paths,
